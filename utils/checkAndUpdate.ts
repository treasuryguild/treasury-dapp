...
export async function checkAndUpdate(myVariable:any, thash: any) {
    ...
    async function checkAddress(myVariable: MyVariableType) {
      ...
      if (data && !["Incoming", "Staking", "Rewards Withdrawal","Minting"].includes(myVariable.txtype)) {
...
function updateWalletBalanceAfterTx(
  ...
  for (let key in totalAmounts) {
      ...
      if (tokenIndex !== -1) {
          ...
          let updatedAmount = 0.00
          if (Object.keys(myVariable.txamounts).length > 5) {
            updatedAmount = parseFloat(walletBalanceAfterTx[tokenIndex].amount) //testing speed problem
          } else {
            updatedAmount = parseFloat(walletBalanceAfterTx[tokenIndex].amount) + totalAmounts[key]; 
          }
